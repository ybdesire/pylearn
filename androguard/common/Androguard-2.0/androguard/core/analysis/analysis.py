      ...
      try:
            ...
            # invoke
                ...
                if self.tainted_packages != None :
                    self.tainted_packages.push_info( method_info[0], TAINTED_PACKAGE_CALL, idx, self, self.method, idx_meth )
            ...
            # new_instance
                ...
                if self.tainted_packages != None :
                    self.tainted_packages.push_info( type_info, TAINTED_PACKAGE_CREATE, idx, self, self.method, None )
...
class PathP(object):
  def __init__(self, access, idx, bb, src_idx, dst_idx) :
    self.access_flag = access
    self.idx = idx
    self.bb = bb;
...
class TaintedPackage(object):
    ...
    def push(self, access, idx, bb, src_idx, dst_idx) :
        p = PathP( access, idx, bb, src_idx, dst_idx )
...
class TaintedPackages(object):
    ...
    def push_info(self, class_name, access, idx, bb, method, idx_method) :
        self._add_pkg( class_name )
        p = self.__packages[ class_name ].push( access, idx, bb, method.get_method_idx(), idx_method )
