...
class APK(object):
    ...
    def __init__(self, filename, raw=False, mode="r", magic_file=None, zipmodule=ZIPMODULE):
        ...
        if zipmodule == 0:
            ...
            # modified by lorin begin @2015/10/8
            #import zipfile
            from androguard.patch import zipfile
            # modified by lorin end @2015/10/8
            self.zip = zipfile.ZipFile(StringIO.StringIO(self.__raw), mode=mode)
    ...
    # added by lorin begin @2015/10/8
    def get_app_locales(self):
        """
            Return the locales of the APK

            :rtype: string
        """
        res_parser = self.get_android_resources()
        return res_parser.get_locales(self.get_package())
    # added by lorin end @2015/10/8
    ...
    def get_app_name(self, locale='\x00\x00'):
        ...
        if app_name.startswith("@"):
            ...
            app_name = ''
            for package_name in res_parser.get_packages_names():
                app_name = res_parser.get_string(package_name, 'app_name', locale)
                if app_name:
                    app_name = app_name[1]
                    break
        return app_name
    ...
    #added by lorin begin @ 2015/10/8
    def get_app_name_by_locale(self, locale):
        """
            Return the appname of the APK
    ...
    #added by lorin begin @ 2015/10/8
    def get_certificate_detail(self, cert):
        """
            Return a certificate info by giving the name in the apk file
        """
        certInfo = dict()
        certInfo['IssuerC'] = cert.issuerC()
        certInfo['IssuerCN'] = cert.issuerCN()
        certInfo['IssuerDN'] = cert.issuerDN()
        certInfo['IssuerE'] = cert.issuerE()
        certInfo['IssuerL'] = cert.issuerL()
        certInfo['IssuerO'] = cert.issuerO()
        certInfo['IssuerOU'] = cert.issuerOU()
        certInfo['IssuerS'] = cert.issuerS()
        certInfo['SubjectC'] = cert.subjectC()
        certInfo['SubjectCN'] = cert.subjectCN()
        certInfo['SubjectDN'] = cert.subjectDN()
        certInfo['SubjectE'] = cert.subjectE()
        certInfo['SubjectL'] = cert.subjectL()
        certInfo['SubjectO'] = cert.subjectO()
        certInfo['SubjectOU'] = cert.subjectOU()
        certInfo['SubjectS'] = cert.subjectS()
        certInfo['validFromStr'] = cert.validFromStr()
        certInfo['validToStr'] = cert.validToStr()
