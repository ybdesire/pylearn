...
class DalvikVMFormat(bytecode._Bytecode):
    ...
    def __init__(self, buff, decompiler=None, config=None, using_api=None):
        ...
        self.lockGetMethodByIdx=threading.RLock()
        self.config = config
        if not self.config:
    ...
    #added by lorin @2016-04-12
    def get_classes_def_dict(self):
        if self.classDefDict is not None:
            return self.classDefDict

        self.classDefDict = {}
        for classDef in self.get_classes():
            # self.classDefDict[hashlib.md5(classDef.get_name()).hexdigest()] = classDef
            self.classDefDict[classDef.get_name()] = classDef
        return self.classDefDict
    #added by lorin end @2016-04-12
